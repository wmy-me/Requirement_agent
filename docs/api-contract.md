...
#### 降级标记：`metadata.degradation`（2026-09-17 · B3.1b 新增）

模型不可用、或主模型失败走了备用模型时，**结论照常产出但必须标记**——
追加实施文档 §4.5：「风险和冲突判断降级后必须静默当作确定结果」。

```json
{"degraded": true,
 "fields": ["risk"],
 "reason": "主模型 openai/gpt-4o-mini 失败，已降级到 deepseek/deepseek-v4-flash"}
```

- `fields` 列出**哪些结论**是降级的（`analysis` / `risk`）
- **只有真降级时才写这个键**，没降级时 `metadata` 里根本没有它
- 三种情况都算降级：① 主模型失败、用了备用模型；② 模型整条链失败、退回启发式规则；
  ③ 压根没配模型
- ⚠️ **审核页要展示它** —— 审核人据此决定要不要更谨慎，而不是把一次规则结论
  当成模型判断
