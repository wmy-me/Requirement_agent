...
**测试基线**：`pytest -q` → **412 passed, 2 skipped**。
> 本文先后写过 73（阶段 2 结束）→ 194 → 251（E 批）→ 311（能力模型六批）→ 393（G 批）→ **412**（T1/T2 与 F 批）。
> 每次加批次都会涨，**以最新一次实测为准**。
...
3. **工具层批次 1** —— `src/requirement_agent/tools/` 下契约 + 注册表 + 10 个工具（**零调用方**）
...
middleware 只加安全响应头与请求日志）。而 **`main.py` 绑的是 `0.0.0.0:8888`**，
**全部写端点（提交 / 审核 / 回滚 / 裁决）都没有鉴权**。

**卡点不是「怎么做」，是部署范围**：只有你能访问，还是局域网 / 公网可达？
后者的话它该排第一。一旦做，**必须豁免飞书 webhook**（§四）——那条路径靠飞书自身验签兜底，
且它是对外暴露的。
...
**N1 · Agent 工具层** —— 你想做的那件。**方案已写：`docs/方案_Agent工具层.md`**（待拍板 4 条）。
