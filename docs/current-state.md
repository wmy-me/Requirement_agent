...
| 8 | **相似度阈值是按小样本校准的，需要重跑** | `ANALYSIS_MODE_THRESHOLDS` 现在的值来自实测：无关内容 0.62~0.74、同文档分片 0.80~0.92，故重复阈值抬到 0.80。但**样本只有 2 条需求**，且「真重复」的上界是用同文档分片近似的。数据量上来后应重跑校准，别把当前数字当永久真理。 |
