...
## 2. 单对话并发隔离（一个对话一次只答一个问题，但可以开新对话并行）

> **状态：已实施**（迁移 `012` + 仓库层 + 路由 409 + 前端按会话隔离）。
> 实施中发现并处理了一个方案里没写到的坑：**僵尸运行**。崩溃/中断留下的
> `running` 行不会自己收尾，而唯一索引会把它当成「有人在用」——
> 于是该对话被**永久**堵死，且没有任何地方会去清理（库里就躺过一条静默 3 天的）。
> 现在加了 `chat_run_stale_timeout_seconds`（默认 1800s）+ `expire_stale_runs()` 自愈，
> 且**前置检查必须先清僵尸再判断**，否则自愈永远走不到。

### 2.0 先说清粒度

- **限制在「对话」这一层**：同一个对话同时只能有一个问题在思考；
- **跨对话不限制**：开一个新对话就能并行问第二个问题，两者互不影响。

这两条是同一个约束的两面 —— 约束的键是 `conversation_id`，**既不是「整个用户」，
也不是「整个进程」**。下面 2.2 与 2.3 分别处理后端与前端，而两边各有一个反向的问题。

### 2.1 现状：**后端零约束，前端反而过度约束**

#### 后端（该管的没管）

- 没有任何会话级锁（全仓只有雪花 ID 与 LLM 统计两把锁，与会话无关）；
- `agent_run` 上的 `uq_run_once (conversation_id, client_message_id)`（`003:47-49`）
  是**幂等去重，不是互斥**：两个不同的 `client_message_id` 打同一对话，两条都会成功、并行跑。

#### 前端（不该管的管了）—— **这才是「开新对话问另一个」走不通的原因**

`state.streaming` 是**页面级单个布尔量**（`app.js:23-28`），不是按会话的。于是流式期间：

| 代码 | 后果 |
|---|---|
| `newChat()` 第一行 `if (state.streaming) return;`（`app.js:678`） | **开不了新对话** |
| `loadSession(id)` 同样 `if (state.streaming) return;`（`app.js:579`） | **也切不到别的已有会话** |
| `runChat` 的发送门禁 `if ((!msg && !attached.length) \|\| state.streaming) return;`（`app.js:478`） | 门禁是全局的，不是当前会话的 |

两条退路都被自己堵死，用户除了「等」没有别的选择。所以本轮要同时修**两个相反的方向**：
后端补上约束，前端**去掉**过度约束。

### 2.2 后端：把约束放进数据库（键是 `conversation_id`）
...
- 同一对话并发插入第二个活跃 run → `23505` 唯一冲突 → 路由捕获后返回 **409**；
- 不同 `conversation_id` 之间**天然不冲突** —— 这正是「跨对话可并行」的实现方式，
  不需要为它额外做任何事；
- **为什么用 DB 约束而不是 `asyncio.Lock`**：锁是进程内的，多 worker 下形同虚设；
  唯一索引天然跨进程、跨实例（outbox 的 `FOR UPDATE SKIP LOCKED` 已是同一思路）。

### 2.3 前端：把 `streaming` 从「一个布尔量」改成「按会话的映射」

```js
// 之前：state.streaming = true / false          —— 页面级，跨会话互相阻塞
// 之后：state.streaming = { [sessionId]: { runId, controller, buffer } }
```

三处配套改动：

1. **门禁按会话**：新增 `isBusy(sessionId)`，只判断**当前会话**有没有在跑。
   `newChat()`（`app.js:678`）与 `loadSession()`（`app.js:579`）里的
   `if (state.streaming) return` **整条删掉** —— 开新对话、切换会话在任何时候都不该被拦。
   `runChat`（`app.js:478`）与发送按钮的 `disabled`（`app.js:467`）改为按当前会话判断。
2. **流与视图解耦**：SSE 回调里先判断「当前视图是不是发起它的那个会话」——
   是就把 token 写进 DOM，不是就只累加到该会话的 `buffer`；切回来时先把 buffer 回填。
   *为什么必须做*：现在回调直接往当前 DOM 追加，一旦允许流式期间切会话，
   上一个会话的回复会写进新会话的视图里。
3. **每个流自带 `AbortController`**：切走或关页面时只中止那一个流，不影响其它会话。

### 2.4 本对话在忙时的提示（主出路是「开新对话」）

- 409 的响应体带上**正在运行的那个 run**（id、stage、开始时间）；
- 前端提示「**本对话**正在思考上一个问题（已完成 2/4 步）」，两个出路：
  - **新建对话去问**（一键 `newChat()`，把输入框里已输入的内容带过去）—— 主出路；
  - **等它跑完**（轮询该 run 状态）；
- 「取消旧 run」降为次要选项（保留，但不再是首选）。

### 2.5 验收

- **跨对话并行**：在对话 A 提问、流式未结束时切到对话 B 提问 →
  两条 run **同时为 `running`**，各自一条；两个视图内容不串（B 里不出现 A 的回复）；
  切回 A 能看到 A 的内容完整（buffer 回填生效）。
- **同一对话互斥**：对同一 `conversation_id` 同时发两条 → 第二条 **409**，
  且库里**不产生第二条 run 行**（靠唯一索引兜底，而不是靠前端自觉）。
- **前端不再自锁**：流式期间「新建对话」可用、会话列表可点、输入框可输入。
...
| 批次 | 内容 | 依赖 | 风险 |
|---|---|---|---|
| **A** | ✅ **已完成**（迁移 `012`）：单对话并发隔离（§2）：唯一索引 + 409 + 僵尸自愈 + 前端按会话隔离 | 无 | 低。纯约束，不改现有流程 |
| **B** | ✅ **已完成**（迁移 `013`）：checkpoint 落库（§1.2a）：stage + checkpoint 两列，各阶段写回。**只做「不丢结果」，不做续跑** | A | 低。只增写入，不改读取路径 |
